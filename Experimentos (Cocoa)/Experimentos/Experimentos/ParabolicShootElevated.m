...
static NSRect funcRect = {-10, -25, 10*2, 25*2};    //Dimensiones para disparos en la tierra
static NSRect funcRectLUNA = {-50, -80, 50*2, 80*2};    //Dimensiones para disparos en la luna
static NSRect funcRectMARTE = {-17, -43, 17*2, 43*2};    //Dimensiones para disparos en marte

@implementation ParabolicShootElevated
...
-(void)drawInRect:(NSRect)b withGraphicsContext:(NSGraphicsContext *)ctx
{
    ...
    if ([planetaExperimento isEqualToString:@"Tierra"]) //representación de gráfica en la tierra
    {
        ...
        anotherPoint.y = [alturaExperimento floatValue] + (([velocidadExperimento floatValue]*sin([anguloExperimento floatValue] * M_PI / 180)*anotherPoint.x)-(GRAV_TIERRA *((anotherPoint.x * anotherPoint.x)/2)));
    ...
    } else if ([planetaExperimento isEqualToString:@"Luna"])    //Representamos la gráfica en la luna
    {
        ...
        anotherPoint.y = [alturaExperimento floatValue] + (([velocidadExperimento floatValue]*sin([anguloExperimento floatValue] * M_PI / 180)*anotherPoint.x)-(GRAV_LUNA *((anotherPoint.x * anotherPoint.x)/2)));
        while (anotherPoint.y > 0)
        {
    ...
    } else if ([planetaExperimento isEqualToString:@"Marte"])   //Representamos la grárica en marte
    {
        ...
        anotherPoint.y = [alturaExperimento floatValue] + (([velocidadExperimento floatValue]*sin([anguloExperimento floatValue] * M_PI / 180)*anotherPoint.x)-(GRAV_MARTE *((anotherPoint.x * anotherPoint.x)/2)));
        while (anotherPoint.y > 0)
        {
